...
const uranai: Command = {
  ...
  async selectMenuHandler(interaction: SelectMenuInteraction) {
    ...
    if (itemParameters.content.length > 16) {
      let newContent = itemParameters.content.replace(
        /(.{5}.*[^一-龠ぁ-ゔァ-ヴーa-zA-Z0-9ａ-ｚＡ-Ｚ０-９々〆〤ヶ])([一-龠ぁ-ゔァ-ヴーa-zA-Z0-9ａ-ｚＡ-Ｚ０-９々〆〤ヶ].{5}.*)/u,
        "$1\n$2",
      );
    ...
    const svg = svgTemplate.replace(/__([A-Z0-9]+)__/g, (_, param) => {
      return parameters[param.toLowerCase() as keyof typeof parameters]
        .toString();
    });
